...
export function getApi(id: string): Promise<WebhookSendRecordDto> {
  return requestClient.get<WebhookSendRecordDto>(
    `/api/webhooks/send-attempts/${id}`,
  );
}
...
export function getPagedListApi(
  ...
  return requestClient.get<PagedResultDto<WebhookSendRecordDto>>(
