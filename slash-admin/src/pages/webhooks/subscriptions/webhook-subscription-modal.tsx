...
const defaultModel: WebhookSubscriptionDto = {
  creationTime: new Date().toISOString(), // DTO expects string usually, check types
  displayName: "",
  extraProperties: {},
  headers: {},
  id: "",
  ...
  webhooks: [],
  webhookUri: "",
} as WebhookSubscriptionDto;

const WebhookSubscriptionModal: React.FC<Props> = ({
  visible,
  onClose,
  onChange,
  subscriptionId,
  ...
  const [formModel, setFormModel] = useState<WebhookSubscriptionDto>({ ...defaultModel });
  ...
  const initData = async () => {
    try {
      const [groupRes, tenantRes] = await Promise.all([
  ...
  const fetchTenants = async (filter?: string) => {
    ...
    const { items } = await getTenantsApi({ filter, maxResultCount: 50 }); // Fetch reasonable amount for dropdown
    return items;
  };
              ...
              <Select
                ...
                options={webhookGroups.map(group => ({
                    ...
                    options: group.webhooks.map(wh => ({
                        ...
                        value: wh.name,
                        // Custom filter prop if needed, Antd usually filters by label
                    }))
                }))}
              />
            </Form.Item>
            ...
            {hasAccessByCodes(["AbpSaas.Tenants"]) && (
              ...
              >
                <Select
                  ...
                  filterOption={false}
                  options={tenants.map(t => ({ label: t.name, value: t.id }))} // name or normalizedName depending on DTO
                />
              </Form.Item>
            )}
