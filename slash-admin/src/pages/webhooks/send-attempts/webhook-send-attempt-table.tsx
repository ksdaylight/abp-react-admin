...
import { Button, Tag, Dropdown, Menu, Modal, Space } from "antd";
...
import { getHttpStatusColor, httpStatusCodeMap } from "@/utils/abp/http-status-code"; // Adjust
...
const WebhookSendAttemptTable: React.FC = () => {
  ...
  const handleSend = (row: WebhookSendRecordDto) => {
    modal.confirm({
      title: $t("AbpUi.AreYouSure"),
      content: $t("WebhooksManagement.ItemWillBeResendMessageWithFormat", { 0: $t("WebhooksManagement.SelectedItems") }),
      onOk: () => resend(row.id),
    });
  };
    ...
    },
    {
      ...
      render: (_, row) => (
        <Tag color={getHttpStatusColor(row.responseStatusCode)}>
          {httpStatusCodeMap[row.responseStatusCode] || row.responseStatusCode}
        </Tag>
      ),
    },
    {
    ...
    <>
      ...
      <ProTable<WebhookSendRecordDto>
        ...
        request={async (params, sorter) => {
          ...
          const sorting = sorter && Object.keys(sorter).length > 0
            ? Object.keys(sorter).map(key => `${key} ${antdOrderToAbpOrder(sorter[key])}`).join(', ')
            : undefined;

          const query = await queryClient.fetchQuery({
            queryKey: ["sendAttempts", params, sorter],
            queryFn: () => getPagedListApi({
