...
import { EditOutlined, DeleteOutlined, PlusOutlined, CheckOutlined, CloseOutlined, EllipsisOutlined, SendOutlined } from "@ant-design/icons";
...
const NotificationDefinitionTable: React.FC = () => {
  ...
  const definitionColumns = [
    ...
    },
    {
      ...
      render: (val: boolean) => val ? <CheckOutlined className="text-green-500" /> : <CloseOutlined className="text-red-500" />,
    },
    {
    ...
    },
    {
      ...
      render: (items: string[]) => (
        <Space wrap>
          {items?.map(p => <Tag key={p} color="blue">{p}</Tag>)}
        </Space>
      ),
    },
    {
