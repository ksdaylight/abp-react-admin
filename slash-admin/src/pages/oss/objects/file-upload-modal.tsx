...
const FileUploadModal: React.FC<Props> = ({ visible, onClose, bucket, path, onFileUploaded }) => {
  ...
  const startUpload = (item: UploadFileItem) => {
    ...
    .catch((err) => {
        setFileList((prev) => prev.map(f => f.id === item.id ? { ...f, status: "error", errorMsg: err.message } : f));
    });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: UploadFileItem) => {
        ...
        if (record.status === "error") return <Tooltip title={record.errorMsg}><Tag color="red">{$t("AbpOssManagement.Upload:Error")}</Tag></Tooltip>;
