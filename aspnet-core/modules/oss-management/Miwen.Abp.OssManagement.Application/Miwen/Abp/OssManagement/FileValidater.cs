...
using Miwen.Abp.OssManagement.Settings;
...
public class FileValidater : IFileValidater, ISingletonDependency
{
    ...
    protected async virtual Task<FileValidation> GetBySettingAsync()
    {
        ...
        var value = await _settingProvider.GetOrNullAsync(AbpOssManagementSettingNames.AllowFileExtensions);
        if (value.IsNullOrWhiteSpace())
        {
            var settingDefintionManager = _serviceProvider.GetRequiredService<ISettingDefinitionManager>();
            var setting = await settingDefintionManager.GetAsync(AbpOssManagementSettingNames.AllowFileExtensions);
            value= setting.DefaultValue;
        }

        return new FileValidation(fileSizeLimited, value.Split(','));
    }
}
